...
A função `class()` revela qual a classe dos objetos
...
### factor
```{r}
kids <- factor(c(1,0,1,0,0,0,1), levels = c(0, 1),
...
#Funções vetoriais
max(x) # valor máximo em x
min(x) # valor mínimo em x
sum(x) # soma todos os valores em x
mean(x) # média de todos os valores em x
median(x) # mediana de todos os valores em x
range(x) # vetor do min(x) e max(x)
var(x) # variância de todos os valores em x
cor(x,y) # correlação entre vetores x e y
quantile(x) # Quantil de todos os valores em x
...
dim(x) 		#mostra as dimensões de uma tabela ou matriz
head(x) 	#mostra as n ('6') primeiras linhas de uma tabela ou matriz
tail(x) 	#mostra as n ('6') últimas linhas de uma tabela ou matriz
nrow(x)   #Número de linhas da matriz
ncol(x)   #Número de colunas da matriz
attr(x)
